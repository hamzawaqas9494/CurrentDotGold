...
async function fetchAndStoreRates() {
  try {
    // Fetch the last stored rate and its timestamp
    const lastRateEntry: any =
      await sql`SELECT * FROM rates ORDER BY date DESC LIMIT 1;`;
    const lastRateTimestamp = lastRateEntry[0]?.date || 0;
    const oneHourAgo = new Date(Date.now() - 60 * 60 * 1000);

    // Check if the last rate was fetched within the last hour
    if (new Date(lastRateTimestamp) > oneHourAgo) {
      console.log("Rates fetched recently, no need to fetch again.");
      return;
    }
    ...
    console.log("Fetched and stored rates:", { goldRate, silverRate });
  } catch (error) {
    console.error("Error processing data:", error);
    throw new Error("Error processing rates");
  }
