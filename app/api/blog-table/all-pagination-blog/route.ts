...
export async function GET(req: NextRequest) {
  try {
    ...
    if (page || blogsPerPage) {
      const paginatedBlogs = await sql`
        SELECT * FROM allblogs ORDER BY ID DESC LIMIT ${blogsPerPage} OFFSET ${offset};
      `;
      responseData.paginatedBlogs = paginatedBlogs.rows;
      const previousLastMonthResult = await sql`
        SELECT * FROM allblogs ORDER BY ID DESC LIMIT 30 OFFSET 3;
      `;
      responseData.previousFive = previousLastMonthResult.rows;
      const totalBlogs = await sql`SELECT COUNT(*) FROM allblogs;`;
      responseData.totalBlogs = totalBlogs.rows[0].count;
    }
